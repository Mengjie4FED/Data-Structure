...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    typename SCHashTable<string, bool>::iterator it;
    for (it = pageVisitedTable.begin(); it != pageVisitedTable.end(); it++) {
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
     ...
     */
    bool res =  whenVisitedTable.keyExists(customer + url);
                                  ...
                                  const string& url) const
{
     ...
     * @todo Implement this function.
     */
    time_t res = whenVisitedTable.find(customer + url);
