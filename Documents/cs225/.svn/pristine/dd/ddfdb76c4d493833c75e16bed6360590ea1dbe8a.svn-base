                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (mergeQueue.empty()){
      ...
      int sinfre = singleQueue.front()->freq.getFrequency();
      int merfre = mergeQueue.front()->freq.getFrequency();
      if ( sinfre <= merfre) {
         smallest = singleQueue.front();
         singleQueue.pop();
      } else {
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     for (unsigned long i = 0; i < frequencies.size(); i++) {
       TreeNode* freNode = new TreeNode(frequencies[i]);
       singleQueue.push(freNode);
     }

     while (singleQueue.size() + mergeQueue.size() > 1)  {

       TreeNode* leftSub = removeSmallest(singleQueue, mergeQueue);
       TreeNode* rightSub = removeSmallest(singleQueue, mergeQueue);
       TreeNode* newNode = new TreeNode(leftSub->freq.getFrequency() + rightSub->freq.getFrequency());
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
         ...
         current = root_; ///jeiorjgoiwejrgoiwjeogjwogjwotjwo
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if (bfile.getNextBit() == 1) {
         Frequency newFre = Frequency(bfile.getNextByte(), 0);
         TreeNode* newLef = new TreeNode(newFre);
         return newLef;
     } else {
         TreeNode* newInternal = new TreeNode(0);
