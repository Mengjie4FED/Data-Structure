...
void SkipList::insert(int key, HSLAPixel value)
{
    ...
    SkipNode *temp = this->find(key); //returns a pointer to the node containing it
    if (temp)                        // if exsits, replace
    {
        temp->value = value;
        return;
    ...
    length++;  // length of list +1

    int newNodeLevel = levelGenerator();

    SkipNode * traverse = head;

    int level=head->nodePointers.size()-1;    // current level
    ...
    for(int i = this->listHeight; i < max(this->listHeight, newNodeLevel); i++)
    {
        head->nodePointers.push_back(SkipPointer());  // both pre. and nest. NULL
    ...
    SkipNode * newNode = new SkipNode(key, value, newNodeLevel);  // NEW!!!!????
