...
class KDTree
{
  public:
    ...
    void KDTreeHelper(vector<Point<Dim>>& points, int left, int right, int curDim);

    int partition(vector<Point<Dim>>& newPoints, int left, int right, int pivotIndex, int curDim);

    void swap(const vector<Point<Dim>>& newPoints, int left, int right);

    Point<Dim> quickSelectKth(vector<Point<Dim>>& newPoints, int left, int right, int k, int curDim);

    void findNNHelp(const vector<Point<Dim>>& points, const Point<Dim>& query, int left, int right, int curDim, Point<Dim>& currentBest) const;
